...
fun EvaluationContainerScreen(
    ...
    BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
        ...
        val paneNavigator = EvalPaneNavigator(realNavigator, viewModel)
                ...
                Row(modifier = Modifier.fillMaxSize()) {
                    visiblePanes.forEach { pane ->
                        Box(
                            ...
                            when (pane) {
                                is EvaluationPane.Menu -> MiuixEvaluationMenuScreen()
                                is EvaluationPane.List -> MiuixEvaluationListScreen(actionUrl = pane.actionUrl, title = pane.title)
                                is EvaluationPane.Detail -> MiuixEvaluationDetailScreen(actionUrl = pane.actionUrl, title = pane.title)
                            }
                        }
                    }
                }
