...
@Composable
fun Material3SunshineContainerScreen(
    ...
    BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
        ...
        val paneNavigator = M3PaneNavigator(realNavigator, viewModel)
...
private fun RenderPane(pane: SunshinePane) {
    when (pane) {
        is SunshinePane.Menu -> Material3SunshineMenuScreen()
        is SunshinePane.List -> Material3SunshineListScreen(typeCode = pane.typeCode, submitUrl = pane.submitUrl)
        is SunshinePane.Detail -> Material3SunshineDetailScreen(item = pane.item)
        is SunshinePane.LostAndFoundList -> Material3LostAndFoundListScreen()
        is SunshinePane.LostAndFoundDetail -> Material3LostAndFoundDetailScreen(item = pane.item)
    }
}

private class M3PaneNavigator(
